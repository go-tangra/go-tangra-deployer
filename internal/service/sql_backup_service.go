...
// the schema-agnostic sqldump engine over deployer's own tables (from ent
// migrate.Tables). Replaces the legacy per-field deployer.service.v1.BackupService,
...
func NewSqlBackupService(ctx *bootstrap.Context) *SqlBackupService {
	...
	return &SqlBackupService{
		log:    ctx.NewLoggerHelper("deployer/service/sql-backup"),
		engine: sqldump.New(dsn, sqldump.Options{Module: "deployer", Tables: tables}),
	}
}
...
func (s *SqlBackupService) ImportBackup(stream commonV1.BackupService_ImportBackupServer) error {
	...
	if opts == nil {
		return errors.New("deployer: first ImportBackup message must carry options")
	}
	...
	if err != nil {
		...
		return stream.SendAndClose(&commonV1.ImportBackupResponse{Success: false, Module: "deployer", Warnings: []string{err.Error()}})
	}
	out := &commonV1.ImportBackupResponse{Success: true, Module: "deployer", Warnings: res.Warnings}
