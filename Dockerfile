...
# go-tangra-deployer (deployer service, go-tangra v4) - standalone image.
...
#     --build-arg APP_VERSION=4.0.0 --build-arg VCS_REF=$(git rev-parse HEAD) -t go-tangra-deployer:dev .
#
...
RUN --mount=type=cache,target=/go/pkg/mod --mount=type=cache,target=/root/.cache/go-build \
    go build -trimpath -tags "ui" -ldflags "-s -w -X main.version=${APP_VERSION}" -o /out/deployersvc ./cmd/deployersvc

FROM alpine:3.20
ARG APP_VERSION=dev
ARG VCS_REF=unknown
LABEL org.opencontainers.image.source="https://github.com/go-tangra/go-tangra-deployer" \
      org.opencontainers.image.title="go-tangra-deployer" \
