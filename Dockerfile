...
COPY services/deployer/ui/ ./services/deployer/ui/
RUN npm run -w services/deployer/ui build

FROM golang:1.26-alpine AS build
RUN apk add --no-cache git ca-certificates
WORKDIR /src
COPY . .
COPY --from=ui /w/services/deployer/ui/dist ./services/deployer/ui/dist
